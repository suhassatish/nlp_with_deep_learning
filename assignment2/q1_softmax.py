...
def softmax(x):
    ...
    exp_scores = tf.exp(x - c)  # using the property that adding a constant to softmax doesnt alter its value.
    # Subtracting max to prevent arithmetic overflow
    sums = tf.reduce_sum(exp_scores, axis=[1], keepdims=True)
    out = exp_scores / sums
